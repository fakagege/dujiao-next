...
type Product struct {
	...
	MaxPurchaseQuantity   int            `gorm:"not null;default:0" json:"max_purchase_quantity"`                             // 单次最大购买数量（0 表示不限）
	FulfillmentType       string         `gorm:"type:varchar(20);not null;default:'manual'" json:"fulfillment_type"`          // 交付类型（auto/manual）
	EnableSecretSelection bool           `gorm:"not null;default:false" json:"enable_secret_selection"`                       // 是否启用自选卡密
	SecretSelectionMarkup Money          `gorm:"type:decimal(20,2);not null;default:0" json:"secret_selection_markup_amount"` // 自选卡密加价
	ManualFormSchemaJSON  JSON           `gorm:"type:json" json:"manual_form_schema"`                                         // 人工交付表单 schema
	ManualStockTotal      int            `gorm:"not null;default:0" json:"manual_stock_total"`                                // 手动剩余库存，-1 表示无限库存
	ManualStockLocked     int            `gorm:"not null;default:0" json:"manual_stock_locked"`                               // 手动库存占用量（待支付）
	ManualStockSold       int            `gorm:"not null;default:0" json:"manual_stock_sold"`                                 // 手动库存已售量
	IsAffiliateEnabled    bool           `gorm:"not null;default:false;index" json:"is_affiliate_enabled"`                    // 是否参与推广返利
	AutoStockAvailable    int64          `gorm:"-" json:"auto_stock_available"`                                               // 自动发货库存可用量（结构字段）
	AutoStockTotal        int64          `gorm:"-" json:"auto_stock_total"`                                                   // 自动发货库存总量（结构字段）
	AutoStockLocked       int64          `gorm:"-" json:"auto_stock_locked"`                                                  // 自动发货库存占用量（结构字段）
	AutoStockSold         int64          `gorm:"-" json:"auto_stock_sold"`                                                    // 自动发货库存已售量（结构字段）
